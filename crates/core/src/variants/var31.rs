//! SteamStub DRM Variant 3.1 unpacker (x86 and x64).
//!
//! Byte-exact port of `Steamless.Unpacker.Variant31.{x86,x64}.Main.cs`.
...
/// SteamStub Variant 3.1 unpacker.
pub struct Variant31 {
    is64: bool,
}

impl Variant31 {
    /// The 32-bit (x86) variant 3.1 unpacker.
    pub const fn x86() -> Self {
        Self { is64: false }
    }

    /// The 64-bit (x64) variant 3.1 unpacker.
    pub const fn x64() -> Self {
        Self { is64: true }
    }
}

impl crate::variants::Unpacker for Variant31 {
    fn name(&self) -> &'static str {
        if self.is64 {
            "SteamStub Variant 3.1.x Unpacker (x64)"
        } else {
    ...
    fn can_process(&self, pe: &PeFile) -> bool {
        ...
        var_3x::get_header_size(pe, Variant::V31, self.is64)
            .map(|s| s != 0 && s == var_3x::expected_size(Variant::V31))
            .unwrap_or(false)
    }

    fn process(&self, pe: &mut PeFile, options: &Options, logger: &dyn Logger) -> Result<()> {
        logger.log(Level::Info, "File is packed with SteamStub Variant 3.1!");

        logger.log(
            Level::Info,
            "Step 1 - Read, decode and validate the SteamStub DRM header.",
        );
        let mut state = var_3x::step1(pe, Variant::V31, self.is64)?;
        ...
        var_3x::step4(pe, options, &mut state, Variant::V31, self.is64, logger)?;

        logger.log(
            Level::Info,
            "Step 5 - Read, decrypt and process code section.",
        );
        var_3x::step5(pe, &mut state, Variant::V31, self.is64, logger)?;

        logger.log(Level::Info, "Step 6 - Rebuild and save the unpacked file.");
        let target = var_3x::step6(pe, options, &state, Variant::V31, self.is64)?;
